...
//    Setup for the MCMC iteration: get values at the starting point
   ...
   // compute starting value for detq:
   ...
   for (unsigned int iter = 0; iter < T; iter++){
      ...
      for (unsigned int i = 0; i < Nsoft; i++){
         ...
         // Independently resample momentum in T_qn
         for ( unsigned int k = 0; k < n; k++){   // Isotropic Gaussian, not tangent
            R[k] = SN(RG);
         }
         
         //       Compute Tqn =  basis for tangent space at qn. To do so, calculate Full SVD of gxiqn = U * S * V^t. Then,
         //       .. Tqn = last d-m columns of U in the Full SVD for gxiqn
         gxiqn  = M.gxi(qn);           // grad xi(qn)
         Agxiqn = M.Agxi(gxiqn);       // add d-n column of zeros to gxiq to get full SVD, needed to get Tq = last d-n columns of U
         svd( Agxiqn, U, s, Vtr);       // Computing the singular values and vectors of gxiq
         
         // FOR LATER : multiply singular values to get detqn = r(qn) = sqrt( det(gxiy^t gxiy) ) = det(S)
         detqn = 1.;
         for ( unsigned long int i = 0; i < m; i++){
            detqn *= s[i];    // detqn = sqrt( det(gxiy^t gxiy) ) = det(S)
         }
         
         //    Build Tqn = matrix for tangent space basis at q
         for ( unsigned long int i = m; i < d; i++){
            unsigned long int k = i-m;
            column(Tqn,k) = column(U,i);
         }      // end of calculation for Tq
         
         pn = Tqn * R;   //   new momentum in the q-tangent space of level Surface S_xi(q)
         ...
         A = exp( 0.5*( (Uq - Uqn) / (eps*eps) ) );  // Metropolis ratio
   ...
   // Metropolis detailed balance check for (q,p) --> (qn, pn) :
      ...
      if ( rtFlag == reverse_check_worked ){  // do Metropolis check only if reverse check was successful
         ...
         // NOTE: we already have detq from the soft move above
      ...
      if ( rtFlag ==  Met_acc_rt) {     //  process an accepted proposal
         ...
         p = - p;   // not necessary : apply ** MOMENTUM REVERSAL ** in the rejection step !!
